...
When we try to handle with a qubit in superposition or not to determine its state using a measurement quantum operator
we make the qubit collapse to one of the state \(|0\rangle\) or \(|1\rangle\) and we call that
...
The information provided shows us the architecture that produces 100\% misses and 100\%. The distinction between both outputs
can be made extracting the information about the sign of the \(|00\dots0\rangle\) state at the end, due the
equation \((-1)^{f(x)}|x\rangle\).
If we find a positive sign, then we know that the architecture produced a 100\% misses and when negative sign is found the
opposite is true.
This kind of verification can be easily accomplished changing measurement operator to Pauli-Y.
