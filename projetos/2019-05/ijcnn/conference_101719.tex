...
\renewcommand{\figurename}{Fig.}
 ...
 After the \(U_f\) operation changes the phase of the states in superposition.
It will make possible for us to verify if a function is constant or not as in equation (1).
The function is constant if the magnitude of the inner product is equal to \(\pm 1\)~\cite{deutsch1992rapid, yanofsky2008quantum}.
However, the function is balanced if the magnitude of the inner product between the initial state \(|\psi\rangle\) and \(U_f\) is zero.
It happens when exactly half part with phase on 1 and the other half on -1.
Consequently, it will make one-half cancel with another half of qubits.
Therefore, it will produce some output different from zero after the
measurement~\cite{deutsch1992rapid,nielsen2002quantum}.
...
\subsection{Verification}\label{subsec:verification}
 ...
 After, we evaluate the outcome applying the $H^{\otimes n}$ in $(x_1, x_2, x_3)$ again and measuring the outcomes.
  ...
  % fazer nova imagem
    \begin{figure}[h!]
    \centering
    \includegraphics[width=9cm, scale=0.5]{}
